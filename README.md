...
Running one long simulation costs a lot of time, and still not guaranteed to run smoothly. Therefore, making the simulation structured with separated each stage restart files can prevent inaccuracy while minimizing wasted time, and running multiple simulations simultanously . The limited computational hardware and comprehension aspire this program runs easily with just a click of button in the Textual UI. As someone who just got introduced with MSD and LAMMPS, this UI can make the learning process easier and more organized simulations runs for Al-Cu diffusion. 
